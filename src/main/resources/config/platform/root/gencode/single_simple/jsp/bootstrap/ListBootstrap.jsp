...
#foreach( $name in $ay_list_column )
	...
	%>
		<tr>
			<td><input value="<%=info.get("${fieldNamePk}")%>" type="checkbox"
				name="name_checkbox_ids"></td>
			<td><a onclick="delRecord('<%=info.get("${fieldNamePk}")%>')" href="#"> 删除
			</a> <a onclick="updateRecord('<%=info.get("${fieldNamePk}")%>')" href="#"> 编辑 </a></td>
#foreach( $name in $ay_list_column )

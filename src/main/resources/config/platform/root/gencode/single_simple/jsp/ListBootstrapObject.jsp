...
<form action="${ay_s}{pageContext.request.contextPath}/${resources_name_all}/${ay_table_name_child}/${ay_table_name_module}/list_object/bootstrap.do"  id="id_form_list" method="post">
...
#foreach( $name in $ay_list_column )
	...
	%>
		<tr>
			<td><input value="<%=info.get${methodNamePk}()%>" type="checkbox"
				name="name_checkbox_ids"></td>
			<td><a onclick="delRecord('<%=info.get${methodNamePk}()%>')" href="#"> 删除
			</a> <a onclick="updateRecord('<%=info.get${methodNamePk}()%>')" href="#"> 编辑 </a></td>
#foreach( $name in $ay_list_column )
	<!--  style="display: none;word-wrap : break-word;" -->
	<td style="word-wrap : break-word;  " id="hit.${name.columnName}">&nbsp;<%=info.get${name.methodName}()%></td>
#end	

 ...
 * 有数据库连接池;
 * 
 * 分页查询(大数据量时，性能比较好,通过最后一条ID的更新时间来查询);
 * 
 * 分页查询(大数据量时，性能比较差,用skip函数);
 * 
 * @Description:
...
public class MongoDBCoreUtil {
	...
	private static String user_database= null;
	...
	protected  static void init() throws Exception {
		...
		if ("true".equals(mongodbLocalStart)) {
			...
			user_database = SysConfig.findInstance().findMap().get("mongodb.local.user_database").toString();
			...
			MongoCredential credential = MongoCredential.createScramSha1Credential(username, user_database,
			...
			// 数据库连接实例
			mongoClient = new MongoClient(serverAddressList, credentialList, mongoClientOptions);
		}
	}
	/**
	 * 初始化
	 ...
	 *             返回类型 void
	 */
	private static void init_v1() throws Exception {
		String mongodbLocalStart = SysConfig.findInstance().findMap().get("mongodb.local.start").toString();
		if ("true".equals(mongodbLocalStart)) {
			// 连接到MongoDB服务 如果是远程连接可以替换“localhost”为服务器所在IP地址
			// ServerAddress()两个参数分别为 服务器地址 和 端口
	...
	public static void setIp(String ip) {
		MongoDBCoreUtil.ip = ip;
	}
	public static int getPort() {
		return port;
	}
	public static void setPort(int port) {
		MongoDBCoreUtil.port = port;
	}
	public static String getUsername() {
		return username;
	}
	public static void setUsername(String username) {
		MongoDBCoreUtil.username = username;
	}
	public static String getPassword() {
		return password;
	}
	public static void setPassword(String password) {
		MongoDBCoreUtil.password = password;
	}
	public static String getDatabase() {
		return database;
	}
	public static void setDatabase(String database) {
		MongoDBCoreUtil.database = database;
	}
	public MongoDatabase findMongoDatabase() {
		// 打开数据库
		// 连接到数据库
	 ...
	 * @throws Exception
	 */
	public List<Map<String, Object>> pageByLast(MongoDatabase mongoDatabase, Class classObj, int pageSize, long lastId)
			throws Exception {
		List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
		// 得到表名
		AnnotationTable table = (AnnotationTable) classObj.getAnnotation(AnnotationTable.class);
		...
		// 1表示正序
		// lt表示小于
		FindIterable findIterable = userCollection.find(Filters.lt("UPDATE_TIME_LONG_", lastId))
				.sort(new BasicDBObject("UPDATE_TIME_LONG_", -1)).limit(pageSize);
		MongoCursor<Document> mongoCursor = findIterable.iterator();
		while (mongoCursor.hasNext()) {
			// log.trace("文档=" + mongoCursor.next());

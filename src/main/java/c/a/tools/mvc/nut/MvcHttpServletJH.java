 ...
 * 
 */
public class MvcHttpServletJH extends HttpServlet {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (StringUtil.isBlank(url)) {
				log.error("mvc出错,找不到参数url");
				this.doExceptionForwardJsp_v2(request, response);
				return;
			}
			mvcAction = MvcConfig.findInstance().findMvcAction(url);
			log.trace("action name=" + mvcAction.getName());
		} catch (Exception e) {
		...
		if (true) {
			try {
				...
				e1.printStackTrace();
				// 写日志
				this.doLog2database_v2(logStr, servletContext, e1, request.getServletPath());
				// 必须重新抛出异常给系统才能跳转
				throw e1;
				...
				e2.printStackTrace();
				// 写日志
				this.doLog2database_v2(logStr, servletContext, e2, request.getServletPath());
				// 必须重新抛出异常给系统才能跳转
				throw e2;
			} catch (Throwable t) {
				...
				t.printStackTrace();
				// 写日志
				this.doLog2database_v2(logStr, servletContext, t, request.getServletPath());
				this.doExceptionForwardJsp_v2(request, response);
				return;
			}
		}
		if (StringUtil.isBlank(returnObject)) {
			// 返回json等等
			if (true) {
				log.trace("[" + request.getServletPath() + "]json跳转");
			}
		} else {
			// 返回result_url
			try {
		...
		if (true) {
			...
			try {
				...
				String logStr = "执行异步方法出错,出错的action=" + request.getServletPath();
				logStr = logFunError + logStr;
				log.error(logStr);
				log.error(logStr, e);
				e.printStackTrace();
				// 写日志
				this.doLog2database_v2(logStr, servletContext, e, request.getServletPath());
			}
		}
		// 对象设置为空，自动回收内存
		baseAction = null;

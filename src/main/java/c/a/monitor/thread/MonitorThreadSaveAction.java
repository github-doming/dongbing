...
public class MonitorThreadSaveAction extends AyAction {
	@Override
	public String execute() throws RuntimeException, Exception {
		// http://localhost:8080/a/c/a/monitor/thread/save.do
		/**
		 * 监控所有线程
		 */
		ThreadPoolExecutor executorMvc = (ThreadPoolExecutor) request.getServletContext()
				.getAttribute(SysConfig.ThreadMvcExecutor);
		executorMvc.setCorePoolSize(201);
		executorMvc.setMaximumPoolSize(301);
		executorMvc.setKeepAliveTime(101, TimeUnit.MILLISECONDS);
		ThreadPoolExecutor executorLocal = (ThreadPoolExecutor) request.getServletContext()
				.getAttribute(SysConfig.ThreadLocalExecutor);
		executorLocal.setCorePoolSize(201);
		executorLocal.setMaximumPoolSize(301);
		executorLocal.setKeepAliveTime(101, TimeUnit.MILLISECONDS);
		...
		// ThreadPoolExecutor executorLocal = (ThreadPoolExecutor)

...
public class FunSingleSimpleGenTreeSimpleListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		 ...
		 * 读取state
		 */
		treeTableParameter.setSql_find_root("SELECT * FROM fun_single_simple where TREE_CODE_='0001'");
		treeTableParameter.setSql("select * from fun_single_simple t1 order by t1.SN_ asc,t1.FUN_SINGLE_SIMPLE_ID_ asc");
		treeTableParameter.setColumnTreeCode("tree_code_".toUpperCase());
		treeTableParameter.setColumnId("FUN_SINGLE_SIMPLE_ID_".toUpperCase());
	...
	public Map<String, Object> tds() throws Exception {
		...
		String sql = "SELECT * FROM fun_single_simple";
		List<Map<String, Object>> listMap = this.findDao().findMapList(sql,
				null);
		for (Map<String, Object> entityMap : listMap) {
			sb.setLength(0);
			// id
			String id =BeanThreadLocal.findThreadLocal().get().find( entityMap.get("FUN_SINGLE_SIMPLE_ID_".toUpperCase()),"");
			// 名称
			if (false) {
				sb.append("<td>");
				sb.append(BeanThreadLocal.findThreadLocal().get().find( entityMap.get("name_".toUpperCase()),""));
				sb.append("</td>");
			}
			// url
			//sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.findThreadLocal().get().find( entityMap.get("url_".toUpperCase()),""));
			sb.append("</td>");
			// 排序
			//sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.findThreadLocal().get().find( entityMap.get("sn_".toUpperCase()),0));
			sb.append("</td>");
			// 操作
			//sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");

...
public class SysUserGenDoubleSimpleService extends BaseService {
	/**
	 * 
	 * 查找实体
	 * 
	...
	if (StringUtil.isBlank(sortOrderName) || StringUtil.isBlank(sortFieldName)) {
			sql_count = "SELECT count(*) FROM SYS_USER s left JOIN sys_group_user t on t.SYS_USER_ID_=s.SYS_USER_ID_ "
					+ "where t.sys_group_id_=" + first$id + " ";
			sql = "select *,s.SYS_USER_ID_ as id FROM SYS_USER s left JOIN sys_group_user t on t.SYS_USER_ID_=s.SYS_USER_ID_ "
					+ "where t.sys_group_id_=" + first$id + " order by s.SYS_USER_ID_ desc";
		} else {
			sql_count = "SELECT count(*) FROM SYS_USER s left JOIN sys_group_user t on t.SYS_USER_ID_=s.SYS_USER_ID_"
					+ "where t.sys_group_id_=" + first$id + "";
			sql = "select *,s.SYS_USER_ID_ as id FROM SYS_USER s left JOIN sys_group_user t on t.SYS_USER_ID_=s.SYS_USER_ID_ "
					+ "where t.sys_group_id_="
					+ first$id
					+ " order by s."

...
public class AppMenuTListAction extends BaseTreeTableRoleAction {
	...
	public TreeTableParameter initParameter() throws Exception {
		...
		treeTableParameter.setSql_find_root("SELECT * FROM app_menu where state_!='DEL' and TREE_CODE_='0001' and TENANT_CODE_='"+tenantCode+"'");
		treeTableParameter
				.setSql("select * from app_menu t1 where state_!='DEL' order by t1.SN_ asc,t1.APP_MENU_ID_ asc");
		...
		treeTableParameter.setColumnName("APP_MENU_NAME_".toUpperCase());
		treeTableParameter.setColumnPath("path_".toUpperCase());
		treeTableParameter.setColumnUrl("admin_url_".toUpperCase());
	...
	public Map<String, Object> tds() throws SQLException {
		...
		for (Map<String, Object> entityMap : listMap) {
			...
			sb.append(BeanThreadLocal.find(entityMap.get("admin_url_".toUpperCase()), ""));
			sb.append("</td>");
			// 排序
			// sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");

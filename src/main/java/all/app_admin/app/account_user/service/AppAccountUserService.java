package all.app_admin.app.account_user.service;
...
public class AppAccountUserService extends BaseService {
	protected Logger log = LogManager.getLogger(this.getClass());
	/**
	 * 查找AppUser
	 * 
	 * @Title: findAppUserByAccountId
	 * @Description:
	 *
	 * 				参数说明
	 * @param id
	 ...
	 *             返回类型 AppUserT
	 */
	public AppUserT findAppUserByAccountId(String id) throws Exception {
		ArrayList<Object> parameters = new ArrayList<Object>();
		String sql = "SELECT 	u.* FROM app_account a LEFT JOIN app_user u ON a.APP_USER_ID_ = u.APP_USER_ID_ WHERE a.APP_ACCOUNT_ID_ = ?";
		parameters.add(id);
		return (AppUserT) this.dao.findObjectUnique(AppUserT.class, sql, parameters);
	}
	/**
	 * 通过用户名，找出用户是否存在
	 * 
	...
	public void delAll(String[] ids) throws Exception {
		if (ids != null) {
			...
			String sql = "update app_account set state_='DEL' where APP_ACCOUNT_ID_ in(" + stringBuffer.toString()
					+ ")";
	...
	public void delAllPhysical(String[] ids) throws Exception {
		if (ids != null) {
			...
			stringBuffer.deleteCharAt(stringBuffer.length() - 1);
			String sql = "delete from app_account where APP_ACCOUNT_ID_ in(" + stringBuffer.toString() + ")";
			dao.execute(sql, null);
		}
	}
	/**
	...
	public PageCoreBean<Map<String, Object>> find(String sortFieldName, String sortOrderName, Integer pageIndex,
		...
		if (StringUtil.isBlank(sortFieldName) || StringUtil.isBlank(sortOrderName)) {
			sql_count = "SELECT count(*) FROM app_account a LEFT JOIN app_user u ON a.APP_USER_ID_ = u.APP_USER_ID_ WHERE a.state_ != 'DEL' AND a.TENANT_CODE_ = ? ";
			sql = "SELECT a.APP_ACCOUNT_ID_,a.ACCOUNT_NAME_, u.NICKNAME_ FROM app_account a LEFT JOIN app_user u ON a.APP_USER_ID_ = u.APP_USER_ID_ WHERE a.state_ != 'DEL' AND a.TENANT_CODE_ = ? ORDER BY a.UPDATE_TIME_ DESC";
		} else {
			sql_count = "SELECT count(*) FROM app_account a LEFT JOIN app_user u ON a.APP_USER_ID_ = u.APP_USER_ID_ WHERE a.state_ != 'DEL' AND a.TENANT_CODE_ = ? ";
			sql = "SELECT a.APP_ACCOUNT_ID_, a.ACCOUNT_NAME_, u.NICKNAME_ FROM app_account a LEFT JOIN app_user u ON a.APP_USER_ID_ = u.APP_USER_ID_ WHERE a.state_ != 'DEL' AND a.TENANT_CODE_ = ?   order by "
		...
		for (Map<String, Object> map : listMap) {
			Object nickNameObject = map.get("NICKNAME_");
	 ...
	 * @throws Exception
	 */
	public PageCoreBean<AppAccountT> findObject(String sortFieldName, String sortOrderName, Integer pageIndex,
		...
		if (StringUtil.isBlank(sortFieldName) || StringUtil.isBlank(sortOrderName)) {
			...
			sql_count = "SELECT count(*) FROM app_account where state_!='DEL'";
			sql = "SELECT * FROM app_account  where state_!='DEL' order by " + sortFieldName + " " + sortOrderName;
		}
		PageCoreBean<AppAccountT> basePage = dao.page(AppAccountT.class, sql, null, pageIndex.intValue(),
	...
	public List<Map<String, Object>> findAll() throws Exception {
		// String sql = "SELECT * FROM app_account where state_!='DEL' order by
		// APP_ACCOUNT_ID_ desc";

...
public class AppAccountUserSaveAction extends BaseAction {
	@Override
	public String execute() throws Exception {
		...
		account = (AppAccountT) RequestThreadLocal.doRequest2Entity(AppAccountTVo.class, AppAccountT.class,
				request);
		...
		if (StringUtil.isBlank(appUserId)) {
			// 判断系统是否存在用户名
			if (userService.isUserByName(account.getAccountName())) {

package all.archit.complex.gy_kaida.layout.check.action;
...
public class CheckAction extends LoginNotAction {
	 ...
	 * @throws Exception
	 */
	private CurrentSysUser findSysUser(String user_name, String user_password) throws Exception {
		...
		// 查找租户的连接
		// String sql =
		// "SELECT * FROM auth_user_info u where u.user_name=? and
		// u.user_password=?";
		String sql = "SELECT * FROM SYS_USER u where u.name=?";
		ArrayList<Object> parameterList = new ArrayList<Object>();
		parameterList.add(user_name);
		// parameterList.add(user_password);
		SysUserT user = (SysUserT) cBaseDao.findObject(SysUserT.class, sql, parameterList);
		if (user == null) {
			return null;
		}

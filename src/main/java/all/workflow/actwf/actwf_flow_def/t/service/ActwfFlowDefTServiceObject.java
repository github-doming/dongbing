...
public class ActwfFlowDefTServiceObject extends BaseService {
	/**
	 * 
	 * 查找实体
	 * 
	 ...
	 * @throws Exception
	 */
	public PageCoreBean<ActwfFlowDefT> find(String sortFieldName, String sortOrderName,
		...
		if (StringUtil.isBlank(sortFieldName)
			...
			sql_count = "SELECT count(*) FROM actwf_flow_def ";
			sql = "SELECT * FROM actwf_flow_def order by UPDATE_TIME_ desc";
		} else {

			sql_count = "SELECT count(*) FROM actwf_flow_def ";
			sql = "SELECT * FROM actwf_flow_def order by " +sortFieldName
					+ " " + sortOrderName;
		}
	 ...
	 * @throws Exception
	 */
	public List<ActwfFlowDefT> findAll() throws Exception {
		//String sql = "SELECT * FROM actwf_flow_def order by ACTWF_FLOW_DEF_ID_ desc";
		String sql = "SELECT * FROM actwf_flow_def order by UPDATE_TIME_ desc";

...
public class TmsProjectTListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		...
		treeTableParameter.setSql("select * from tms_project t1 where state_!='DEL' order by t1.SN_ asc,t1.TMS_PROJECT_ID_ asc");
	...
	public Map<String, Object> tds() throws SQLException {
		...
		for (Map<String, Object> entityMap : listMap) {
			sb.setLength(0);
			// id
			String id =BeanThreadLocal.find( entityMap.get("TMS_PROJECT_ID_".toUpperCase()),"");
			// 名称
			if (false) {
				sb.append("<td>");
			...
			sb.append("<input type='button' onclick=\"newRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\"  value='添加下级项目'></input>");
			sb.append("</td>");

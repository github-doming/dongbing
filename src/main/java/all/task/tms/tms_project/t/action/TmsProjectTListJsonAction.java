...
public class TmsProjectTListJsonAction extends BaseAction {
	@Override
	public String execute() throws Exception {
		...
		List<TmsProjectT> menuList = jdbcTool.findObjectList(TmsProjectT.class, conn, sql, null);
		List<TmsProjectT> treeEasyuiMenuBeanList = tree(menuList, "0");
		String jsonStr = JsonThreadLocal.findThreadLocal().get().list2json(treeEasyuiMenuBeanList);
		response.getWriter().print(jsonStr);
		return null;
	}
	public List<TmsProjectT> tree(List<TmsProjectT> easyuiMenuBeanList, String id) {
		// 递归转化为树形
		List<TmsProjectT> treeEasyuiMenuBeanList = new ArrayList<TmsProjectT>();
		for (TmsProjectT easyuiMenuBean : easyuiMenuBeanList) {
			TmsProjectT treeEasyuiMenuBean = new TmsProjectT();
			treeEasyuiMenuBean.setTmsProjectId(easyuiMenuBean.getTmsProjectId());
			...
			if (id.equals(easyuiMenuBean.getParent())) {
				treeEasyuiMenuBean.setChildren(tree(easyuiMenuBeanList, treeEasyuiMenuBean.getTmsProjectId()));
	...
	public String execute2() throws Exception {
		...
		for (Map<String, Object> map : mapList) {
			map.put("sysId", map.get("TMS_PROJECT_ID_"));

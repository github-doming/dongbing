...
public class TmsTaskTListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		 ...
		 * 读取state
		 */
		treeTableParameter.setSql_find_root("SELECT * FROM tms_task where state_!='DEL' and TREE_CODE_='0001'");
		treeTableParameter
				.setSql("select * from tms_task t1 where state_!='DEL' order by t1.SN_ asc,t1.TMS_TASK_ID_ asc");
		treeTableParameter.setColumnTreeCode("tree_code_".toUpperCase());
		treeTableParameter.setColumnId("TMS_TASK_ID_".toUpperCase());
	...
	public Map<String, Object> tds() throws SQLException {
		...
		for (Map<String, Object> entityMap : listMap) {
			...
			sb.append("<input type='button' onclick=\"newRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\"  value='添加下级任务'></input>");
			sb.append("</td>");

...
public class TmsFunctionTListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		 ...
		 * 读取state
		 */
		treeTableParameter.setSql_find_root("SELECT * FROM tms_function where state_!='DEL' and TREE_CODE_='0001'");
		treeTableParameter.setSql(
				"select * from tms_function t1 where state_!='DEL' order by t1.SN_ asc,t1.TMS_FUNCTION_ID_ asc");
		treeTableParameter.setColumnTreeCode("tree_code_".toUpperCase());
		treeTableParameter.setColumnId("TMS_FUNCTION_ID_".toUpperCase());
	...
	public Map<String, Object> tds() throws SQLException {
		...
		String sql = "SELECT f.TMS_FUNCTION_ID_,f.PARENT_,f.PATH_,	f.NAME_,f.API_DEF_,"
				+ " f.API_EXAMPLE_,f.API_PARAMETER_,f.API_RETURN_," + " u.APP_USER_NAME_ AS BACKEND_DEVELOPER_NAME_ "
				+ "FROM tms_function f LEFT JOIN app_user u " + "ON f.BACKEND_DEVELOPER_ID_ = u.APP_USER_ID_";
		List<Map<String, Object>> listMap = this.findDao().findMapList(sql, null);
		for (Map<String, Object> entityMap : listMap) {
			sb.setLength(0);
			// id
			String id = BeanThreadLocal.find(entityMap.get("TMS_FUNCTION_ID_".toUpperCase()), "");
			// 名称
			if (false) {
				sb.append("<td>");
			...
			sb.append(BeanThreadLocal.find(entityMap.get("BACKEND_DEVELOPER_NAME_".toUpperCase()), ""));
			sb.append("</td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.find(entityMap.get("API_DEF_".toUpperCase()), ""));
			sb.append("</td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.find(entityMap.get("API_EXAMPLE_".toUpperCase()), ""));
			sb.append("</td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.find(entityMap.get("API_PARAMETER_".toUpperCase()), ""));
			sb.append("</td>");
			sb.append("<td class='class_crud_td_white'>");
			sb.append(BeanThreadLocal.find(entityMap.get("API_RETURN_".toUpperCase()), ""));
			sb.append("</td>");
			// 常用操作
			sb.append("<td class='class_crud_td_white'>");
			sb.append("<input type='button' onclick=\"viewRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\" value='查看'></input>");
			sb.append("</td>");
			// 操作
			// sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");
			...
			sb.append("<input type='button' onclick=\"editRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\" value='编辑'></input></br>");
			sb.append("<input type='button' onclick=\"delRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\" value='删除'></input></br>");
			sb.append("<input type='button' onclick=\"newRecord(' " + id
					+ "   ');\"  class=\"btn btn-link\"  value='添加下级功能接口'></input></br>");
			sb.append("</td>");

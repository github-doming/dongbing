...
public class SysGroupMenuCxRoleListAction extends BaseTreeTableAction {
	...
	public TreeTableParameter initParameter() throws Exception {
		...
		treeTableParameter
				.setSql("select * FROM SYS_MENU  t1 where PERMISSION_GRADE_>="
						+ this.findCurrentSysUser().getPermissionGrade()
						+ "  order by t1.SN_ asc,t1.SYS_MENU_ID_ asc");
	...
	public Map<String, Object> tds()throws Exception{
		...
		for (Map<String, Object> jsonMap : listMap) {
			...
			if (false) {
				sb.append("<td>");
				sb.append((String) jsonMap.get("SYS_MENU_NAME_"));
				sb.append("</td>");
			}
			// url
			sb.append("<td>");
			...
			// sb.append("abc");
			if(false){
				sb.append("<button onclick=\"editRecord('" + id
						+ "');\"  class=\"btn btn-link\">编辑</button>");
				sb.append("<button onclick=\"delRecord(' " + id
						+ "   ');\"  class=\"btn btn-link\">删除</button>");
				sb.append("<button onclick=\"newRecord(' " + id
						+ "   ');\"  class=\"btn btn-link\">添加下级菜单</button>");
				sb.append("</td>");
			}

...
public class SysDictTreeTListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		...
		treeTableParameter.setSql("select * from sys_dict_tree t1 where state_!='DEL' order by t1.SN_ asc,t1.SYS_DICT_TREE_ID_ asc");
	...
	public Map<String, Object> tds() throws SQLException {
		...
		for (Map<String, Object> entityMap : listMap) {
			sb.setLength(0);
			// id
			String id =BeanThreadLocal.find( entityMap.get("SYS_DICT_TREE_ID_".toUpperCase()),"");
			// 名称
			if (false) {
				sb.append("<td>");
			...
			sb.append(BeanThreadLocal.find( entityMap.get("SYS_DICT_TREE_CODE_".toUpperCase()),""));
			sb.append("</td>");
			// 排序
			//sb.append("<td>");
			sb.append("<td class='class_crud_td_white'>");

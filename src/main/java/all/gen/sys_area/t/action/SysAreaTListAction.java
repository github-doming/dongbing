...
public class SysAreaTListAction extends BaseTreeTableRoleAction {
	@Override
	public TreeTableParameter initParameter() throws Exception {
		...
		treeTableParameter.setSql_find_root("SELECT * FROM sys_area where state_!='DEL' and TREE_CODE_='0001'");
		treeTableParameter.setSql("select * from sys_area t1 where state_!='DEL' order by t1.SN_ asc,t1.SYS_AREA_ID_ asc");
		
		treeTableParameter.setColumnTreeCode("tree_code_".toUpperCase());
		treeTableParameter.setColumnId("SYS_AREA_ID_".toUpperCase());
	...
	public Map<String, Object> tds() throws SQLException {
		...
		for (Map<String, Object> entityMap : listMap) {
			sb.setLength(0);
			// id
			String id =BeanThreadLocal.find( entityMap.get("SYS_AREA_ID_".toUpperCase()),"");
			// 名称
			if (false) {
				sb.append("<td>");

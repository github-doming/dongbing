...
/**
 * @Description: 号码追踪冷热排名
 * @Author: admin1
 * @Date: 2020-09-17 10:35
 */
public class TrackNumberHotAndCold extends PlanBase {
	public static TrackNumberHotAndCold getInstance() {
		return new TrackNumberHotAndCold();
	}
	@Override
	...
	public String betContent(JSONObject groupData, long fundTh, Object expandInfo, Map<String, Object> historyMap) {
		...
		int hotRank = groupData.getInteger("rank");
		//投注位置或遗漏排名为空
		if (bet <= 0 || hotRank <= 0) {
		...
		try {
			String[][] hotAndColdData = CacheTool.getNumberHotAndCold(gameCode(),handicapCode(),drawType(),period().findLotteryPeriod());
			...
			String betStr = hotAndColdData[bet-1][hotRank-1];

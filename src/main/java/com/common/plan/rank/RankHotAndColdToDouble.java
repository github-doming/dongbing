 ...
 * @Version: v1.0
 */
public class RankHotAndColdToDouble extends PlanBase {
	public static RankHotAndColdToDouble getInstance() {
		return new RankHotAndColdToDouble();
	}
	...
	public String betContent(JSONObject groupData, long fundTh, Object expandInfo, Map<String, Object> historyMap) {
		try {
			...
			for (Map.Entry<String, Object> planGroup : groupData.entrySet()) {
				...
				//解析数据
				ranks = betData.getString("rank").split(",");
				//投注位置或遗漏排名为空
				if (StringTool.isEmpty(betData.get("bet")) || betData.getString("bet").length() != 1 || ContainerTool.isEmpty(ranks)) {
					log.info("解析数据出错,方案组信息：{}", betData);
					continue;
				}
				ranksInt = NumberTool.intValue(ranks);
				assert ranksInt != null;
				int bet = betData.getInteger("bet");

 ...
 * @Version: v1.0
 */
public class FollowTwoSide extends PlanBase {

	public static FollowTwoSide getInstance() {
		return new FollowTwoSide();
	}
	...
	public boolean valiData(PlanItem planItem) {
		...
		return !expand.containsKey("UN_BETTING_");
	}
	...
	public String betContent(JSONObject groupData, long fundTh, Object expandInfo, Map<String, Object> historyMap) {
		try {
			...
			switch (gameCode()) {
				...
				case SELF_SSC_5:
				case COUNTRY_SSC:
					return betContentBall(state, numIndex, typeIndex, game, fundTh, expandInfo);
				case XYNC:
				case GDKLC:
	...
	private String betContentHappy(boolean state, int numIndex, int typeIndex, Game game, long fundTh) {
		...
		return result.append("|").append(fundTh).append("#").toString();
	}


	private String betContentBall(boolean state, int numIndex, int typeIndex, Game game, long fundTh, Object expandInfo) {
		StringBuilder result = new StringBuilder();
		int number;
		if (numIndex == 5) {
			//总和

...
/**
 * @Description: 跟上去双面一起翻倍
 * @Author: admin
 * @Date: 2020-06-11 16:44
 * @Email: job.dongming@foxmail.com
 * @Version: v1.0
 */
public class FollowTwoSideToDouble extends PlanBase {

	public static FollowTwoSideToDouble getInstance() {
		return new FollowTwoSideToDouble();
	}
	...
	public boolean valiData(PlanItem planItem) {
		...
		//{"UN_BETTING_":"CLOSE","FUNDS_OPTIONS_":"FLAT_TO_WIN"}
		JSONObject expand=JSONObject.parseObject(planItem.getExpandInfo());
		return !expand.containsKey(PlanInfoEnum.UN_BETTING_.name()) || !expand.containsKey(PlanInfoEnum.FUNDS_OPTIONS_.name());
	}
		...
		try {
			...
			for (Map.Entry<String, Object> planGroup : groupData.entrySet()) {
				...
				switch (gameCode()) {
					...
					case SELF_SSC_5:
					case COUNTRY_SSC:
						betContentBall(result,state, numIndex, typeIndex, game, fundTh, expandInfo);
						break;
					case XYNC:
					case GDKLC:
	...
	private void betContentBall(StringBuilder result, boolean state, int numIndex, int typeIndex, Game game, long fundTh, Object expandInfo) {
		int number;
		if (numIndex == 5) {
			//总和
			number = Integer.parseInt(baseData[0]) + Integer.parseInt(baseData[1]) + Integer.parseInt(baseData[2])
		...
		switch (typeIndex) {
			...
			case 2:
				//龙虎
				if (numIndex != 6|| ContainerTool.isEmpty(expandInfo)) {
					throw new RuntimeException("非法的排序捕捉，索引为" + numIndex);
				}
				...
				//龙虎和不投注
				if(StateEnum.OPEN.name().equals(expand.getString(PlanInfoEnum.UN_BETTING_.name()))&&"和".equals(dTSum)){
					return;
				}

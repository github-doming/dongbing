...
public abstract class BaseMvcData  extends BaseMvcTransactions {
	...
	protected String getTenant() throws Exception {
		if (tenant == null) {
			tenant = BeanThreadLocal
					.find(SysConfig.findInstance().findMap().get(SysConfig.commLocalTenant), IbsConfig.TENANT_CODE);
		}
		return tenant;
	}
	...
	private Map<String, Object> findDateMapPlus() {
		...
		if (StringTool.notEmpty(json)) {
			...
			log.info(IbsConfig.LOG_SIGN + "没有找到数据，所请求的request=" + getParamsString());
			MvcActionTool.returnCode400();
		}
	...
	void findDateMap(Map<String, Object> map) {
		...
		try {
			...
			log.error(IbsConfig.LOG_SIGN + "读取配置失败");
		...
		if (dataObject != null) {
			...
			if (encryptDate) {
				...
				try {
					...
					log.warn(IbsConfig.LOG_SIGN + "data解密失败,dataObject=" + dataObject.toString());
				...
				if (StringTool.notEmpty(decode)) {
					...
					log.warn(IbsConfig.LOG_SIGN + "data解密失败,解密的data为空");
				...
				try {
					...
					log.warn(IbsConfig.LOG_SIGN + "data转化失败,data解码=" + decode);
					MvcActionTool.returnCode404();
					return;
				}
			} else {

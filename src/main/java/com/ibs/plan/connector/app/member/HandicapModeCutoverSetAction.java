 ...
 * @Version: v1.0
 */
@ActionMapping(value = "/ibs/app/member/modeCutoverSet", method = HttpConfig.Method.POST)
public class HandicapModeCutoverSetAction extends CommCoreAction {
	...
	public Object run() throws Exception {
		...
		String desc = this.getClass().getName().concat("修改真实模拟切换");
		...
		try {
			...
			String modeState = dataMap.getOrDefault("MODE_CUTOVER_STATE_", "").toString();
			...
			modeCutover.setUpdateTime(nowTime);
			modeCutover.setUpdateTimeLong(System.currentTimeMillis());
			modeCutover.setDesc(desc);
			modeCutoverService.update(modeCutover);

			new IbspHmSetService().updateEntity(handicapMemberId,"",modeState,nowTime,desc);

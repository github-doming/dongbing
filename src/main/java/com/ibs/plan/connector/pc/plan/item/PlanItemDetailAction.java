package com.ibs.plan.connector.pc.plan.item;
 ...
 * @Version: v1.0
 */
@ActionMapping(value = "/ibs/pc/plan/itemDetail", method = HttpConfig.Method.POST)
public class PlanItemDetailAction extends CommCoreAction {
	...
	public Object run() throws Exception {
		...
		try {
			...
			String planGroupActiveKey = dataMap.getOrDefault("PLAN_GROUP_ACTIVE_KEY_", "").toString();
			...
			String expandInfo = dataMap.getOrDefault("EXPAND_INFO_", "").toString();
			...
			planItem.setPlanGroupActiveKey(planGroupActiveKey);
			planItem.setExpandInfo(expandInfo);

			Plan plan=PlanUtil.Code.valueOf(planCode).getPlan();
			//数据校验和过滤
			if(plan.valiData(planItem)){
	 ...
	 * @param content           消息内容
	 */
	public static void putPlanItem(String planCode, Map<String, Object> planItemTableInfo, JSONObject content) throws SQLException {
	...
	private void savePlanLog(PlanItem planItem) throws Exception {
		...
		logPlan.setHandleContent("PROFIT_LIMIT_MAX_:".concat(profitLimitMax).concat(",LOSS_LIMIT_MIN_:").concat(lossLimitMin)
				.concat(",PLAN_GROUP_DATA_:").concat(planGroupData).concat(",FUNDS_LIST_:").concat(planItem.getFundsList()).concat(",PLAN_GROUP_ACTIVE_KEY_:")
				.concat(planItem.getPlanGroupActiveKey()).concat(",FOLLOW_PERIOD_:") + planItem.getFollowPeriod() + ",MONITOR_PERIOD_:" + planItem.getMonitorPeriod() + ",BET_MODE_:"
				.concat(planItem.getBetMode()).concat(",FUND_SWAP_MODE_:").concat(planItem.getFundSwapMode()).concat(",PERIOD_ROLL_MODE_:").concat(planItem.getPeriodRollMode())
				.concat(",EXPAND_INFO_").concat(planItem.getExpandInfo()));

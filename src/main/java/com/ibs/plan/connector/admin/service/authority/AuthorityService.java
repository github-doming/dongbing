package com.ibs.plan.connector.admin.service.authority;
 ...
 * @Version: v1.0
 */
public class AuthorityService extends BaseServiceProxy {
	...
	public PageCoreBean<Map<String, Object>> listAllUser(String userId, String userName, Integer pageIndex,
		...
		String sqlCount =
				"SELECT COUNT(*) FROM ibsp_user iu LEFT JOIN app_user_group aug ON iu.APP_USER_ID_ = aug.APP_USER_ID_ "
		...
		parameterListCount.add(IbsStateEnum.DEL.name());
		parameterListCount.add(IbsStateEnum.DEL.name());

		String sql =
				"SELECT iu.NICKNAME_,ag.APP_GROUP_NAME_,iu.STATE_, iu.APP_USER_ID_ FROM ibsp_user iu "
						...
						+ " WHERE aug.APP_GROUP_ID_ IN (SELECT APP_GROUP_ID_ FROM app_group WHERE PERMISSION_GRADE_ > ?)"
						+ " AND iu.STATE_ != ? AND aug.STATE_ != ? AND ag.STATE_ != ? ";
	...
	public Map<String, Object> findLoginInfo(String userId) throws SQLException {
		String sql = "SELECT distinct APP_USER_ID_,IP_,UPDATE_TIME_ FROM ibsp_user_token WHERE APP_USER_ID_ = ? AND STATE_ != ?";
		List<Object> parameterList = new ArrayList<>(2);
		parameterList.add(userId);
		parameterList.add(IbsStateEnum.DEL.name());
		return super.findMap(sql, parameterList);
	...
	public Map<String, Object> findUser(String userId) throws SQLException {
		String sql = "SELECT iu.NICKNAME_,ag.APP_GROUP_NAME_,ag.APP_GROUP_CODE_,iu.STATE_ FROM ibsp_user iu "
				+ " LEFT JOIN app_user_group aug ON iu.APP_USER_ID_ = aug.APP_USER_ID_ "
				+ " LEFT JOIN app_group ag ON aug.APP_GROUP_ID_ = ag.APP_GROUP_ID_ "
				+ " WHERE iu.APP_USER_ID_ = ? AND iu.STATE_ != ? AND aug.STATE_ != ? AND ag.STATE_ != ?";
		List<Object> parameterList = new ArrayList<>(4);
		parameterList.add(userId);
		parameterList.add(IbsStateEnum.DEL.name());
		parameterList.add(IbsStateEnum.DEL.name());
	...
	public void saveUserRole(String appUserId, String roleId, String userId, Date nowTime) throws Exception {
		...
		userGroupT.setState(IbsStateEnum.OPEN.name());
	...
	public void updateUser(String userName, String state, String userId, String updateId) throws SQLException {
		String sql = "UPDATE ibsp_user set UPDATE_TIME_ = ?,UPDATE_TIME_LONG_ = ?,STATE_ = ? ";
		List<Object> parameterList = new ArrayList<>(4);
		parameterList.add(updateId);
		parameterList.add(new Date());
		parameterList.add(System.currentTimeMillis());
	...
	public void deleteUser(String userId) throws SQLException {
		...
		parameterList.add(userId);
		parameterList.add(IbsStateEnum.DEL.name());
		super.execute(String.format(sql, "ibsp_user"), parameterList);
		super.execute(String.format(sql, "ibsp_user_token"), parameterList);

...
import com.ibm.old.v1.client.core.job.bet.BetSgWinJob;
...
import java.util.*;
/**
 * @Description: 投注进SgWin盘口
 * @Author: zjj
 * @Date: 2019-08-10 15:09
 * @Email: 543974681@qq.com
 * @Version: v1.0
 */
public class BetSgWinController implements ClientExecutor {
	...
	@Override public JsonResultBeanPlus execute(JSONObject msgObj, IbmClientExistHmT existHmT, String messageId)
		...
		if (Integer.parseInt(hmBetInfo.get("BET_SECOND_").toString()) != 0) {
			...
			String sealTime = configTService
					.findValue(IbmHandicapEnum.SGWIN.name().concat("_").concat(gameCode).concat("_SEAL"));
		...
		if (IbmMethodEnum.MANUAL_BET.name().equals(msgObj.get("method"))) {
			...
			if (saveBetItem(hmBetInfo, msgObj)) {
				BetSgWinJob betSgWinJob=new BetSgWinJob();
				return betSgWinJob.execute(existHmId, gameCode, period);
			}
	...
	private boolean saveBetItem(Map<String, Object> hmBetInfo, JSONObject msgObj) throws Exception {
		...
		if (ContainerTool.notEmpty(errorBetItem)) {
			...
			//获取更新异常的投注信息，返回到主服务器
			MessageReceipt.messageReceiptBetError(errorInfo, gameCode, period, IbmHandicapEnum.SGWIN.name());
		}
		return true;
	}
	/**
	...
	private Map<String, List<String>> saveBallBetItem(JSONArray betItems, JSONObject betLimit, int splitTwoSideAmount,
		...
		for (int i = 0; i < betItems.size(); i++) {
			...
			//各个盘的容器
			List<String> doubleSidesItem = new ArrayList<>();
			List<String> ballNOItem = new ArrayList<>();
			List<String> sumDTItem = new ArrayList<>();

			for (String bet : bets) {
				//投注限额
				String limitCode = SgWinConfig.BALL_LIMIT_CODE.get(bet);
				...
				if (funds < limitMin) {
					...
					if (SgWinConfig.DOUBLE_SIDES_CODE.containsKey(bet)) {
						setSplitItem(doubleSidesItem, code, splitNumberAmount, splitTwoSideAmount, bet, funds, limitMin,
								limitMax);
					} else if (SgWinConfig.BALL_NO_CODE.containsKey(bet)) {
						setSplitItem(ballNOItem, code, splitNumberAmount, splitTwoSideAmount, bet, funds, limitMin,
								limitMax);
					} else if (SgWinConfig.SUM_DT_CODE.containsKey(bet)) {
						setSplitItem(sumDTItem, code, splitNumberAmount, splitTwoSideAmount, bet, funds, limitMin,
								limitMax);
					}
					...
					if (SgWinConfig.DOUBLE_SIDES_CODE.containsKey(bet)) {
						doubleSidesItem.add(bet.concat("#").concat(String.valueOf(funds)));
					} else if (SgWinConfig.BALL_NO_CODE.containsKey(bet)) {
						ballNOItem.add(bet.concat("#").concat(String.valueOf(funds)));
					} else if (SgWinConfig.SUM_DT_CODE.containsKey(bet)) {
						sumDTItem.add(bet.concat("#").concat(String.valueOf(funds)));
					}
				}
			}
			//投注code
			if (ContainerTool.notEmpty(doubleSidesItem)) {
				betCode.put("dobleSides", JSONArray.fromObject(doubleSidesItem));
			} else if (ContainerTool.notEmpty(ballNOItem)) {
				betCode.put("ballNO", JSONArray.fromObject(ballNOItem));
			} else if (ContainerTool.notEmpty(sumDTItem)) {
				betCode.put("sumDT", JSONArray.fromObject(sumDTItem));
			}
	...
	private JsonResultBeanPlus autoBet(int betSecond, long drawTime, long timeDistance) throws Exception {
		if (timeDistance - betSecond <= 0) {
			...
			// 定时投注
			return QuartzIbmUtil.betJob(existHmId, IbmHandicapEnum.SGWIN, gameCode, period, betTime);
		}
	}
}

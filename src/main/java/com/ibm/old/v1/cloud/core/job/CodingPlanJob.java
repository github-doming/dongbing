 ...
 * @Description: 编码方案信息
 * @Author: Dongming
 * @Date: 2019-07-26 16:13
 * @Email: job.dongming@foxmail.com
 * @Version: v1.0
 */
public class CodingPlanJob extends BaseCommJob {
	IbmGameEnum game;
	int sleepTime;
	public CodingPlanJob(IbmGameEnum game, int sleepTime) {
		this.game = game;
	...
	@Override public void executeJob(JobExecutionContext context) throws Exception {
		...
		GameTool.find20HistoryData(period, game);
		for (int i = 0; i < 10; i++) {
			if (GameTool.get20HistoryData(game).size() == 20) {
				break;
			}
			Thread.sleep(sleepTime);
			GameTool.find20HistoryData(period, game);
		}
		GameTool.findHotAndCold(PeriodTool.findLotteryPeriod(game), game);

		//编码当前方案信息
		for (Map<String, Object> plan : planInfo) {
			for (String handicapId : handicapIds) {
				executorService.execute(
						new CodingPlanItemThread(gameId, plan.get("IBM_PLAN_ID_").toString(), handicapId, period, game,

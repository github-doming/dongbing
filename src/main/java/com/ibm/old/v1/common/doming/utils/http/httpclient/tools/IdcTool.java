...
public class IdcTool {
	 ...
	 * @return 登陆页面信息
	 */
	public static HttpResult getLogin(HttpClientConfig httpConfig, String handicapUrl, String handicapCaptcha,
		...
		HttpResult loginInfo = getLogin(httpConfig, projectHost, loginDateMap, memberAccount, memberPassword,
				verifyCode);
	 ...
	 * @return 登陆信息
	 */
	public static HttpResult getLogin(HttpClientConfig httpConfig, String hostUrl,
		...
		try {
			...
			result = HttpClientUtil.findInstance().post(httpConfig.url(url).map(join));
			...
			if ("302".equals(result.getStatusCode().toString())) {
				log.error("url= " + url);
			...
			if ("404".equals(result.getStatusCode().toString())) {
				log.error("url= " + url);
	...
	public static JSONObject getOddInfo(HttpClientConfig httpConfig, String projectHost, String wagerRoundNo,
			String gameno, List<String> gameBetCode, int... index) throws InterruptedException, IOException {
		if (index.length == 0) {
			index = new int[1];
		}
		if (index[0] > MAX_RECURSIVE_SIZE) {
	...
	public static String betValiToken(HttpClientConfig httpConfig, Map<String, String> userInfo, String[] betItems,
			int... index) throws InterruptedException, IOException {
		if (index.length == 0) {
			index = new int[1];
		}
		if (index[0] > MAX_RECURSIVE_SIZE) {
	...
	public static String orderHandler(HttpClientConfig httpConfig, Map<String, String> userInfo, String gameno,
			String period, String betItem) throws IOException {
		Map<String, Object> join = new HashMap<>(5);
	...
	public static String betting(HttpClientConfig httpConfig, String gameNo, Map<String, String> userInfo,
			String betItemArr, String period, String token) throws IOException {
		JSONObject entity = new JSONObject();
		entity.put("gameno", gameNo);
	...
	public static String getProfitLoss(HttpClientConfig httpConfig, String gameno, String projectHost, String date,
			String isJs, int curPage, int pageSize, int... index) throws InterruptedException, IOException {
		if (index.length == 0) {
			index = new int[1];
		}
		if (index[0] > MAX_RECURSIVE_SIZE) {
	...
	public static String getMemberManage(HttpClientConfig httpConfig, String projectHost, int... index)
		...
		String html=null;
		try {
			html = HttpClientUtil.findInstance().getHtml(httpConfig.url(projectHost + "/opadmin/membermanage.aspx"));
	...
	public static JSONArray getMemberInfoList(HttpClientConfig httpConfig, String projectHost, String agentAccount,
			String curMemberName, int... index)
			throws InterruptedException {
		if (index.length == 0) {
			index = new int[1];
		}
	...
	public static String getDate(HttpClientConfig httpConfig, String projectHost, int... index)
		...
		String html;
		try {
			html=HttpClientUtil.findInstance().postHtml(httpConfig.url(projectHost.concat("/opadmin/mreport_new.aspx/GetDate")).map(map));
			if(StringTool.isEmpty(html)){
				log.error("获取会员信息失败", html);
				Thread.sleep(SLEEP);
	 ...
	 * @return 错误信息
	 */
	public static IbmHcCodeEnum loginError(String html) {
		if (StringTool.contains(html, "您输入的帐号与密码不匹配")) {
			...
			return IbmHcCodeEnum.IBM_403_LOGIN_OFTEN;
			...
			return IbmHcCodeEnum.IBM_403_LOGIN_FAIL;
		} else {
	...
	public static Map<String, List<Map<String, String>>> getBetResultData(List<Map<String, String>> errorData,
		...
		if (StringTool.isContains(html, "【", "】")) {
			...
			for (Element table : tables) {
				...
				//移除成功的投注项
				boolean state = IdcTool.removeBetInfo(betItems, betItem);
				if (state) {

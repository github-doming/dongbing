 ...
 * @Version: v1.0
 */
@ActionMapping(name = "refreshBetInfo", value = "/ibm/pc/ibm_handicap_game/refreshBetInfo.dm") public class RefreshBetInfoAction
	...
	@Override public String run() throws Exception {
		...
		try {
			...
			//获取当期投注总额和当期投注总数
			execBetItemTService.findSum(handicapMemberId, PeriodTool.findPeriod(gameCode), tableName, data);
			...
			if (ContainerTool.notEmpty(newBetInfoList)) {
				int	waitBet = listNewFormat(newBetInfoList, betMerger);
				data.put("newBetInfoList", newBetInfoList);
				data.put("waitBet", waitBet);
			}
			...
			if (ContainerTool.notEmpty(oldBetInfoList)) {
				int	okBet = listFormat(oldBetInfoList, betMerger);
				data.put("oldBetInfoList", oldBetInfoList);
				data.put("okBet", okBet);
			}
	 ...
	 * @return 等待投注数
	 */
	private int listNewFormat(List<Map<String, Object>> betInfoList, String betMerger) {
		int waitBet = 0;
		for (Map<String, Object> betInfo : betInfoList) {
			Map<String, String> betContentMap = new HashMap<>(5);
			//处理投注项信息格式BET_CONTENT_
			...
			betInfo.put("FUND_", Math.floor(NumberTool.doubleT(betInfo.get("FUND_T_").toString())));
	 ...
	 *
	 * @param betInfoList 投注信息列表
	 * @param betMerger   是否投注合并
	 * @return 完成投注数
	 */
	private int listFormat(List<Map<String, Object>> betInfoList, String betMerger) {
		int okBet = 0;
		for (Map<String, Object> betInfo : betInfoList) {
			String execState = betInfo.get("EXEC_STATE_").toString();
			if (StringTool.notEmpty(betInfo.get("DESC_"))) {

 ...
 * 
 * @Description: 自动停止投注
 * @date 2019年3月4日 上午11:35:31 
 * @author wck
 * @Email: 810160078@qq.com
 * @Version v1.0
 *
 */
public class IbmHandicapAutoStopAction extends BaseAppAction {
	...
	public String run() throws Exception {
		...
		// 自动停止投注
		String betAutoStop = BeanThreadLocal.find(dataMap.get("BET_AUTO_STOP_"), "");
		// 自动停止投注时间
		String betAutoStopTime = BeanThreadLocal.find(dataMap.get("BET_AUTO_STOP_TIME_"), "0");
		
		if(StringTool.isEmpty(handicapMemberId, gameCode)||"0".equals(betAutoStopTime)){
			jrb.putEnum(IbmCodeEnum.IBM_401_NOT_FIND_DATA);
			jrb.putSysEnum(IbmCodeEnum.CODE_401);
			return this.returnJson(jrb);
		}
		if(!IbmStateEnum.hmGameSetState(betAutoStop)){
		...
		try {
			...
			String handicapId = ibmHandicapMemberTService.findHandicapId(handicapMemberId);
			// 判断盘口用户是否存在
			...
			if (ibmHmGameSetT == null) {
				...
				ibmHmGameSetT.setBetAutoStop(betAutoStop);
				ibmHmGameSetT.setBetAutoStopTime(DateTool.getTime(new SimpleDateFormat("HH:mm:ss").format(Long.parseLong(betAutoStopTime))));
			...
			ibmLogHandicapMemberT.setDesc(",BET_AUTO_STOP_:"
					+ ibmHmGameSetT.getBetAutoStop() + ",BET_AUTO_STOP_TIME_:"
					+ ibmHmGameSetT.getBetAutoStopTime().toString());

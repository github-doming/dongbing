 ...
 * @Version: v1.0
 */
@ActionMapping(value = "/ibm/admin/manage/core/quartz/pause")
	...
	public Object run() throws Exception {
		...
		try {
			String url = SysConfig.findInstance().findMap()
					.getOrDefault("ibm.server.local", IbmMainConfig.SERVER_LOCAL).toString()+"/all/job/trigger/pause.do";
			JSONObject json = new JSONObject();
			if (StringTool.notEmpty(quartzTriggerIds)) {
				json.put("quartzTriggerIds", quartzTriggerIds);

 ...
 * @Version: v1.0
 */
public class FollowBetInfoController implements ClientExecutor {
	private static final Logger log = LogManager.getLogger(FollowBetInfoController.class);
	private String existHmId;
	private GameUtil.Code gameCode;
	private Object period;
	private String betContent;
	private double funds;
	public FollowBetInfoController(String existHmId, GameUtil.Code gameCode, Object period, String betContent,
			double funds) {
	...
	@Override public JsonResultBeanPlus execute(JSONObject msgObj) throws Exception {
		...
		if (sealTime <= 0) {
			...
			if (sealTime <= 0) {
                sendErrorReceipt(msgObj.get("HA_FOLLOW_BET_ID_").toString(), "已封盘");
		...
		if (usedQuota - funds <= 0) {
			...
			if (usedQuota - funds <= 0) {
				//余额不足，发送错误消息
                sendErrorReceipt(msgObj.get("HA_FOLLOW_BET_ID_").toString(), "余额不足");
		...
		if (sealTime <= betSecond || betSecond == 0) {
			if (IbmStateEnum.OPEN.equal(setInfo.get("BET_MERGER_"))) {
				CustomerTool.putBetItem(existHmId, gameCode, betContent.split("#"),msgObj.get("HA_FOLLOW_BET_ID_").toString());
				...
				//投注
				followBet(existHmId, gameCode, code, period);
				return bean.success();
			}
		}
	...
	private void saveHmBet(JSONObject msgObj, double betRate) throws Exception {
		...
		if (ContainerTool.isEmpty(betCode)) {
			sendErrorReceipt(msgObj.get("HA_FOLLOW_BET_ID_").toString(), "投注项错误");
			return;
		}
		//获取投注详情信息
		...
		hmBet.setHaFollowBetId(msgObj.get("HA_FOLLOW_BET_ID_"));
	 ...
	 * @param period       期数
	 */
	public static void followBet(String existHmId, GameUtil.Code gameCode, HandicapUtil.Code handicapCode,
			Object period) throws Exception {
		CurrentTransaction.transactionCommit();
		getExecutor().execute(() -> {
			try {
				BetJob.getJob(handicapCode).followBet(existHmId, gameCode, period).execute(null);
			} catch (Exception e) {
				log.error("投注失败", e);
			}
		});

...
/**
 * @Description: 抓取HQ盘口投注项
 * @Author: null
 * @Date: 2020-04-02 17:25
 * @Version: v1.0
 */
public class GrabBetHqJob extends GrabBetJob {
    ...
    private void execute() throws Exception {
        ...
        //获取期数字符串
        Object platformPeriod = PeriodUtil.getPeriod(handicapCode, gameCode, period);
        // 获取跟投会员的未结算摘要信息
        ...
        if (ContainerTool.isEmpty(bean.getData())) {
            ...
            log.debug(String.format("用户【%s】的【%s】盘口，抓取【%s】游戏【%s】期,获取投注摘要为空", existHaId, handicapCode.name(),
        ...
        if (memberInfo == null) {
            for (Map.Entry<String, SummaryInfo> entry : member.entrySet()) {
                ...
                putBetMap(betMap, entry.getValue(),platformPeriod);
            }
        } else {
            for (String memberAccount : memberInfo.keySet()) {
                ...
                putBetMap(betMap,member.get(memberAccount),platformPeriod);
            }
        }
        //获取到的数据为空
    ...
    private void putBetMap(Map<String,double[][]> betMap, SummaryInfo summaryInfo,Object platformPeriod) {
        ...
        if (ramMemberInfo==null) {
            bean=agentUtil().getBetDetail(existHaId, gameCode, platformPeriod, summaryInfo.getMemberId(), "");
        }else{
            ...
            log.error("代理【"+existHaId+"】期数【"+platformPeriod+"】重复抓取,旧内存信息【"+ramMemberInfo+"】,新内存信息【"+summaryInfo);
            bean=agentUtil().getBetDetail(existHaId, gameCode, platformPeriod, summaryInfo.getMemberId(), ramMemberInfo.getOddNumber());
        }
        if (!bean.isSuccess()) {
        ...
        log.info("代理【"+existHaId+"】期数【"+platformPeriod+"】抓取,旧内存信息【"+ramMemberInfo+"】,新内存信息【"+summaryInfo);

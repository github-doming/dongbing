 ...
 * @Version: v1.0
 */
public class BetBwJob extends BetJob {

    @Override
    public void executeJob(JobExecutionContext context) throws Exception {
        try {
            handicapCode=HandicapUtil.Code.BW;
            ...
            betProcess();
            log.info("定时投注Bw盘口用户【" + existHmId + "】游戏【" + gameCode + "】期数【" + period + "】投注完成");
        } catch (Exception e) {
            log.error("定时投注Bw盘口用户【" + existHmId + "】游戏【" + gameCode + "】期数【" + period + "】异常:" + e.getMessage());
            throw e;
        }
    }
    /**
    ...
    private void bet(MemberOption memberOption, List<String> betItems, String betType, double fund,
        ...
        //获取赔率信息
		memberOption.oddsInfo(gameCode,"ALL");
        //投注
        JsonResultBeanPlus bean=memberOption.betting(gameCode,period,"ALL",betItems,new ArrayList<>());
        if (!bean.isSuccess()) {

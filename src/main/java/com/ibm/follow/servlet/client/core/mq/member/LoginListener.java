...
public class LoginListener extends BaseCommMq {
	...
	@Override public String execute(String message) throws Exception {
		log.info(getLog("会员登录，接收的消息是：" + message));
		//消息预处理
		if (!valiParameter(message)) {
			bean.putSysEnum(CodeEnum.CODE_401);
			bean.setRequestType(IbmStateEnum.FAIL.name());
			log.error(getLog("会员登录，返回的结果是：".concat(bean.toJsonString())));
			RabbitMqTool.sendMemberReceipt(bean.toJsonString(), "login");
			return null;
		}
		try {
			//消息接收-process
			...
			switch (method) {
				...
				default:
					...
					log.error(getLog( "错误的会员登录方法接收".concat(method.name())));
					return null;
			}
			...
			log.error(getLog("会员登录,处理错误:".concat(e.getMessage())));
			throw e;
		} finally {
			// 消息处理完成
            ...
            log.info(getLog("会员登录,处理完成，返回的结果是：".concat(bean.toJsonString())));
			RabbitMqTool.sendMemberReceipt(bean.toJsonString(), "login");
		}
		return null;
	}

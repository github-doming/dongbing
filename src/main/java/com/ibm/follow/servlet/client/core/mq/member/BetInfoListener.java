...
import com.ibm.follow.servlet.client.core.controller.bet.ManualBetInfoController;
 ...
 * @Version: v1.0
 */
public class BetInfoListener extends BaseCommMq {
	...
	@Override public String execute(String message) throws Exception {
		...
		try {
			...
			switch (method) {
				case MANUAL_BET:
					//手动投注
					bean = new ManualBetInfoController(existHmId, gameCode, period, betContent, funds).execute(msgObj);
					break;
				case FOLLOW_BET:
					//跟随投注
					bean = new FollowBetInfoController(existHmId, gameCode, period, betContent, funds).execute(msgObj);
					break;
				default:

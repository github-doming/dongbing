...
import com.ibm.common.utils.http.utils.agent.NewCcAgentUtil;
...
/**
 * @Description: 抓取新CC盘口投注项
 * @Author: null
 * @Date: 2020-04-27 18:11
 * @Version: v1.0
 */
public class GrabBetNewCcJob extends GrabBetJob {
    ...
    @Override public void executeJob(JobExecutionContext context) throws Exception {
        handicapCode= HandicapUtil.Code.NEWCC;
    ...
    private void execute() throws Exception {
        ...
        // 获取跟投会员的未结算摘要信息
        NewCcAgentUtil agentUtil= NewCcAgentUtil.findInstance();
        JsonResultBeanPlus bean =agentUtil.getBetSummary(existHaId,gameCode,period,date);
        if (!bean.isSuccess()) {
        ...
        if (memberInfo == null) {
            ...
            for (String memberAccount : memberInfo.keySet()) {
                ...
                putBetMap(agentUtil,betMap,member.get(memberAccount),date);
            }
        }
        //获取到的数据为空
    ...
    private void putBetMap(NewCcAgentUtil agentUtil, Map<String,double[][]> betMap, SummaryInfo summaryInfo, String date) {
        ...
        if (ramMemberInfo==null) {
            bean=agentUtil.getBetDetail(existHaId,gameCode,"",summaryInfo,period,date);
        }else{
            ...
            bean=agentUtil.getBetDetail(existHaId,gameCode,ramMemberInfo.getOddNumber(),summaryInfo,period,date);
        }
        if (!bean.isSuccess()) {

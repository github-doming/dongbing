...
public class ManageListener extends BaseCommMq {
	...
	@Override public String execute(String message) throws Exception {
		...
		if (!valiParameter(message)) {
			...
			log.error(getLog("会员管理，返回的结果是：".concat(bean.toJsonString())));
			bean.setToken(eventId);
			bean.setCommand(method.name());
			bean.setRequestType(IbmStateEnum.FAIL.name());
			RabbitMqTool.sendMemberReceipt(bean.toJsonString(), "manage");
			return null;
		}
		try {
			//消息接收-process
			...
			switch (method) {
				...
				default:
					...
					log.error(getLog( "错误的会员管理方法接收".concat(method.name())));
					return null;
			}
            ...
            log.info(getLog("会员管理,处理完成，返回的结果是：".concat(bean.toJsonString())));
			RabbitMqTool.sendMemberReceipt(bean.toJsonString(), "manage");
		}
		return null;
	}

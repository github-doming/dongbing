...
import com.ibm.follow.servlet.cloud.core.controller.process.OpenClientController;
...
public class ManageListener extends BaseCommMq {
	...
	@Override public String execute(String message) throws Exception {
		log.info(getLog("代理管理信息，接收的消息是：" + message));
		//消息预处理
		if (!valiParameter(message)) {
			bean.putSysEnum(CodeEnum.CODE_401);
			log.error(getLog("代理管理信息，处理的结果是：".concat(bean.toJsonString())));
			return null;
		}
		try {
			// 信息处理
			...
			switch (command) {
				case OPEN:
					execute=new OpenClientController();
					break;
				case CLOSE:
				...
				default:
					...
					log.error(getLog( "错误的代理管理信息方法接收".concat(command.name())));
					return null;
			}
            bean = execute.execute(msgObj, requestType);
			log.debug(getLog("代理管理信息，处理完成，处理的结果是：".concat(bean.toJsonString())));
		} catch (Exception e) {
			log.error(getLog("代理管理信息,处理错误:".concat(e.getMessage())));
			throw e;
		}

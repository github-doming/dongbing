...
public class ClientOpenThread extends CommEventThread {
	...
	@Override public String execute(String ignore) throws Exception {
		...
		// 执行事件，获取结果
		JSONObject result = openClient(eventId);
	...
	private JSONObject openClient(String eventId) throws Exception {
		...
		if (StringTool.notEmpty(eventInfo.get("CLIENT_CODE_"))) {
			...
			if (ContainerTool.isEmpty(clientInfo)) {
				log.info("开启客户端失败，客户机容量已满,请联系客服人员");
				return resultJson;
			}
	 ...
	 */
	private void saveExistInfo(IbmTypeEnum customerType, JSONObject content, JSONObject data, String clientId,
	 ...
	 * @return 发送结果
	 */
	private String sendOpenClientInfo(IbmTypeEnum customerType, JSONObject content, String clientCode, String eventId)
			throws Exception {
		//存入事件id

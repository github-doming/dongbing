 ...
 * @Version: v1.0
 */
public class LoginControlThread extends ControlEventThread {
	...
	@Override public String execute(String ignore) throws Exception {
		try {
			...
			while (isRun && runCount++ < 1000) {
				//获取新的事件
				Map<String, Object> newInfo = loginService.findNewEvent();
				...
				//开启-打开客户端子线程
				executor.execute(new LoginThread(newInfo));
			}
		} catch (Exception e) {
			IbmLogEvent logEvent = new IbmLogEvent();
			logEvent.setThreadName(Thread.currentThread().getName() + ":" + EventDriver.Code.LOGIN.name());
			logEvent.setEventCode("Login");
			logEvent.setExecuteClassCode(this.hashCode());
			logEvent.setErrorContext(e.getMessage());
			new IbmLogEventService().saveError(logEvent);
			log.error("用户登录事件错误", e);
		} finally {
			EventDriver.register().removeEvent(EventDriver.Code.LOGIN, this);
		}
		return null;
	}
}

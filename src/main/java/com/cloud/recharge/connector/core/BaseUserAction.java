...
public abstract class BaseUserAction extends BaseMvcData {
	 ...
	 * @return 访问对象
	 */
	public CardAdmin findUser() throws Exception {
		logAccessUrl();
	 ...
	 * @return 访问对象
	 */
	private CardAdmin findUserPlus() {
		findJsonPlus();
	...
	private void findUser(Map<String, Object> map) {
		...
		// 找到 CardAdmin
		try {
			user = new CardAdminService().findUserByToken(token);
			if (ContainerTool.isEmpty(user)||!StateEnum.OPEN.name().equals(user.getUserState())) {
				MvcActionTool.returnCode403Disable();
				return;
			}
		} catch (Exception e) {
			log.error("查找用户失败", e);

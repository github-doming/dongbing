...
<form action="${pageContext.request.contextPath}/ibm/admin/ibm_game/list.do"  id="id_form_list" method="post">
<table id="topic_title" style="width:90%;table-layout:fixed; margin: 0px auto;"  id="id_table" class="table  table-bordered table-hover">
	...
	<tr>
		...
		<th  colspan="5" style="text-align: right">
			<input value="${ibmGameId}" name="ibm_game_id" type="hidden"/>
			<input id="seach_text" style="width:300px;height:20px" name="gameName" id="id_input_new" placeholder="请输入游戏名称" type="text" value="${gameName}">
...
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','GAME_NAME_');"
				 style="cursor: pointer;">游戏名称</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','GAME_CODE_');"
				 style="cursor: pointer;">游戏CODE</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','ICON_');"
				 style="cursor: pointer;">游戏图片路径</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','HANDICAP_WORTH_T_');"
				 style="cursor: pointer;">游戏价值</th>
		</tr>
	</thead>
	<tbody>
		<%

...
<center>
	<div style="color:white;width:100%;height:100px;background-color:rgb(29,41,57);padding-top:20px">
		<span style="font-size:30px;">角色管理</span>
	</div>
</center>
<form action="${pageContext.request.contextPath}/com/ibm/admin/ibm_role/w/list.do"  id="id_form_list" method="post">
<table id="topic_title" style="width:90%;table-layout:fixed; margin: 0px auto;"  id="id_table" class="table  table-bordered table-hover">
	...
	<tr>
		...
		<th  colspan="5" style="text-align: right">
			...
			<input id="seach_text" style="width:300px;height:20px" name="roleName" id="id_input_new" placeholder="请输入角色名" type="text" value="${roleName}"> 
...
<th
				...
				title="按[角色名称]排序" style="cursor: pointer;">角色名称</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','ROLE_CODE_');"
				title="按[角色CODE]排序" style="cursor: pointer;">角色CODE</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','ROLE_LEVEL_');"
				title="按[角色等级]排序" style="cursor: pointer;">角色等级</th>
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','ONLINE_NUMBER_MAX_');"
				title="按[允许最大在线数]排序" style="cursor: pointer;">允许最大在线数</th>
		</tr>
	</thead>
	<tbody>
		<%
	...
	%>
<tr>
	<td><input value="<%=info.get("IBM_ROLE_ID_")%>" type="checkbox"
		name="name_checkbox_ids"></td>
	<td><a onclick="delRecord('<%=info.get("IBM_ROLE_ID_")%>')" href="#"> 删除
	</a> <a onclick="updateRecord('<%=info.get("IBM_ROLE_ID_")%>')" href="#"> 编辑 </a></td>
	<td style="word-wrap : break-word;  " id="hit.ROLE_NAME_">&nbsp;<%=info.get("ROLE_NAME_")%></td>
	<td style="word-wrap : break-word;  " id="hit.ROLE_CODE_">&nbsp;<%=info.get("ROLE_CODE_")%></td>
	<td style="word-wrap : break-word;  " id="hit.ROLE_LEVEL_">&nbsp;<%=info.get("ROLE_LEVEL_")%></td>
...
function delAllRecord(id){
	var url = "${pageContext.request.contextPath}/com/ibm/admin/ibm_role/w/delAll.do";
...
function delRecord(id){
	if(confirm("确定要删除吗？"))
	{
		var url = "${pageContext.request.contextPath}/com/ibm/admin/ibm_role/w/del.do?id="+id;
...
function updateRecord(id){
	var url= "${pageContext.request.contextPath}/com/ibm/admin/ibm_role/w/form.do?id="+id;
 	window.location.href=url;
}
/**
 * 
 ...
 */
function newRecord(){
 	var url= "${pageContext.request.contextPath}/com/ibm/admin/ibm_role/w/form.do";
 	window.location.href=url;
}
	</script>
</html>
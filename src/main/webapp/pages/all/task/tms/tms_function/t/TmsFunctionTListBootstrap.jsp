 ...
 */
 function editRecord(id){
	 	var url= "${pageContext.request.contextPath}/all/task/tms/tms_function/t/form/bootstrap.do?action=edit&&id="+id;
 ...
 function newRecord(parent_id){
 	var url= "${pageContext.request.contextPath}/all/task/tms/tms_function/t/form/bootstrap.do?action=new&&parent_id="+parent_id;
 	window.location.href=url;
 }
 /**
  * 删除
 ...
 function delRecord(id){
		...
		{
			var url = "${pageContext.request.contextPath}/all/task/tms/tms_function/t/del/bootstrap.do?id="+id;
		 	window.location.href= url;
		}
		else
		{
...
<table style="word-wrap : break-word;  table-layout:fixed" class="table  table-bordered table-hover" border="1">
	...
	<thead>
		<tr>
			<th width="13%"><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.name%></th>
			<th><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.backendDeveloperName%></th>
			<th width="13%"><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.apiDef%></th>
			<th width="13%"><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.apiExample%></th>
			<th width="13%"><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.apiParameter%></th>
			<th width="13%"><%=all.task.tms.tms_function.t.alias.TmsFunctionTAlias.apiReturn%></th>
			<th width="13%">常用操作</th>
			<th width="13%">操作</th>
		</tr>
	</thead>

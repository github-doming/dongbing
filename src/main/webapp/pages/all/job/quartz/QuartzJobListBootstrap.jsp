...
<body>
	...
	<form
		action="${pageContext.request.contextPath}/all/job/quartz/job/list.do"
		...
		<table style="width: 100%; table-layout: fixed" id="id_table"
			...
			<tbody>
				...
				<tr>
					...
					<td><a onclick="pause('<%=info.get("JOB_NAME_")%>')" href="#">
							暂停</a> <a onclick="resume('<%=info.get("JOB_NAME_")%>')" href="#">
							恢复</a> <a onclick="delRecord('<%=info.get("JOB_NAME_")%>')" href="#">
							删除 </a></td>
...
<script type="text/javascript">
	...
	function delAllRecord(id) {
		var url = "${pageContext.request.contextPath}/all/job/quartz/job/delAll.do";
	...
	function delRecord(id) {
		if (confirm("确定要删除吗？")) {
			var url = "${pageContext.request.contextPath}/all/job/quartz/job/del.do?id="
					+ id;
			window.location.href = url;
		} else {
		}

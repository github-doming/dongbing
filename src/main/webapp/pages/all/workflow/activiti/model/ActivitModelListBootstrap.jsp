...
<body>
	...
	<form
		action="${pageContext.request.contextPath}/all/workflow/activiti/model/list.do"
		...
		<table style="width: 100%; table-layout: fixed" id="id_table"
			...
			<thead>
				<tr>
					...
					<th
						onclick="ayPageOrderEvent('${pageContext.request.contextPath}','DESC_');"
						title="按MetaInfo排序" style="cursor: pointer;">MetaInfo</th>
			...
			<tbody>
				<%
					java.util.ArrayList<org.activiti.engine.repository.Model> listMap = (java.util.ArrayList<org.activiti.engine.repository.Model>) request
							.getAttribute("list");
					for (org.activiti.engine.repository.Model info : listMap) {
				%>
				<tr>
					<td><input value="<%=info.getId()%>" type="checkbox"
						name="name_checkbox_ids"></td>
					<td>
					<a onclick="deployRecord('<%=info.getId()%>')" href="#">
							部署 </a> 
								<a onclick="editRecord('<%=info.getId()%>')" href="#">
							编辑 </a> 
							<a onclick="delRecord('<%=info.getId()%>')" href="#"> 删除
					</a></td>
					...
					<!--  style="display: none;word-wrap : break-word;" -->
					<td style="word-wrap: break-word;" id="hit.DESC_">&nbsp;<%=info.getKey()%></td>
					<td style="word-wrap: break-word;" id="hit.DESC_">&nbsp;<%=info.getMetaInfo()%></td>
...
<script type="text/javascript">
	 ...
	 * 
	 deploy记录
	 */
	function deployRecord(id) {
		if (confirm("确定要部署吗？")) {
			var url = "${pageContext.request.contextPath}/all/workflow/activiti/model/deploy.do?id="
					+ id;
	 ...
	 */
	function newRecord(){
	 	var url= "http://localhost:9080/wf/index/create";
	 	window.location.href=url;
	}
	/**
	 * 
	...
	function delRecord(id) {
		if (confirm("确定要删除吗？")) {
			var url = "${pageContext.request.contextPath}/all/workflow/activiti/model/del.do?id="
					+ id;
			window.location.href = url;
		} else {
		}
	...
	function delAllRecord(id) {
		var url = "${pageContext.request.contextPath}/all/workflow/activiti/model/delAll.do";

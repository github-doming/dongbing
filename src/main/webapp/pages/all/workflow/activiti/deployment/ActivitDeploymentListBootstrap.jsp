...
<body>
	...
	<form
		action="${pageContext.request.contextPath}/all/workflow/activiti/deployment/list.do"
		...
		<table style="width: 100%; table-layout: fixed" id="id_table"
			...
			<thead>
				<tr>
					...
					<th
						...
						title="按[<%=all.gen.sys_quartz_trigger.t.alias.SysQuartzTriggerTAlias . jobName%>]排序"
						style="cursor: pointer;">id</th>
					<!-- style="display: none; cursor: pointer;" -->
					<th
						onclick="ayPageOrderEvent('${pageContext.request.contextPath}','JOB_CLASS_NAME_');"
						title="按[<%=all.gen.sys_quartz_trigger.t.alias.SysQuartzTriggerTAlias . jobClassName%>]排序"
			...
			<tbody>
				<%
					java.util.ArrayList<org.activiti.engine.repository.Deployment> listMap = (java.util.ArrayList<org.activiti.engine.repository.Deployment>) request
							.getAttribute("list");
					for (org.activiti.engine.repository.Deployment info : listMap) {
				%>
				<tr>
					...
					<td>
						 ...
						 <a
						onclick="delRecord('<%=info.getId()%>')" href="#"> 删除 </a></td>
...
<script type="text/javascript">
	 ...
	 */
	function pause(id) {
		var url = "${pageContext.request.contextPath}/all/job/quartz/job/pause.do?id="
				+ id;
	 ...
	 */
	function resume(id) {
		var url = "${pageContext.request.contextPath}/all/job/quartz/job/resume.do?id="
				+ id;
		window.location.href = url;
	}
	/**
	...
	function delAllRecord(id) {
		var url = "${pageContext.request.contextPath}/all/workflow/activiti/deployment/delAll.do";
	...
	function delRecord(id) {
		if (confirm("确定要删除吗？")) {
			var url = "${pageContext.request.contextPath}/all/workflow/activiti/deployment/del.do?id="
					+ id;
			window.location.href = url;
		} else {
		}

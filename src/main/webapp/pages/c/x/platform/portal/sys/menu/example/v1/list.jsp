...
<form id="id_form_list"
	action="${pageContext.request.contextPath}/menu/list.do" method="post">
  ...
  <tbody>
    <tr>
      ...
      <div  align="center" valign="middle">
				菜单名称
	<input  type="text" name="menu_name" v	value="${requestScope.value_menu_name}">
...
<table id="id_table" class="table  table-bordered table-hover">
  ...
  <thead>
 <tr>
		<th><input id="ifAll" onclick=Ay.table.checkAll();
		...
		<th onclick="ayPageOrderEvent('${pageContext.request.contextPath}','tree_code');" style="cursor: pointer;">tree_code</th>
		...
		<th class="class_color"><!--隐藏列事件--> <select
			onchange="Ay.table.hidden_column(this,'id_table',2);">
  ...
  <tbody>
	...
	<tr>
		<td><input value="<%=info.getSysMenuId()%>" type="checkbox"
			name="checkboxIds"></td>
		<td><button class="btn btn-link" onclick="delRecord('<%=info.getSysMenuId()%>')" href="#"> 删除
		</button> 
			 <a 

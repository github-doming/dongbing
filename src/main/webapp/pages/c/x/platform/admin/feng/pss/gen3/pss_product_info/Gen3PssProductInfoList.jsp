...
<form action="${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/pss/gen3/pss_product_info/list.do"  id="id_form_list" method="post">
...
<th
				...
				title="按[<%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . id%>]排序" style="cursor: pointer;"><%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . id%></th>
		<!-- style="display: none; cursor: pointer;" -->
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','name');"
				title="按[<%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . name%>]排序" style="cursor: pointer;"><%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . name%></th>
	<th class="class_color"><!--隐藏列事件--> <select  style="width:60px; " 
				onchange="ayTableHiddenColumn(this,'id_table',2);">
				<option value=" &nbsp;" selected="selected">&nbsp;</option>
	<option value="hit.id"><%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . id%></option>
	<option value="hit.name"><%=c.x.platform.feng.feng.pss.gen3.pss_product_info.alias.Gen3PssProductInfoAlias . name%></option>
			</select> <!--隐藏列事件--></th>
		</tr>
	</thead>
	<tbody>
...
function delAllRecord(id){
	var url = "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/pss/gen3/pss_product_info/del_all.do";
...
function delRecord(id){
	if(confirm("确定要删除吗？"))
	{
		var url = "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/pss/gen3/pss_product_info/del.do?id="+id;
...
function updateRecord(id){
	var url= "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/pss/gen3/pss_product_info/form.do?id="+id;
	var first$tree$id=document.getElementById("id_first$tree$id").value;
	url=url+"&&first$tree$id="+first$tree$id;
 	window.location.href=url;
}
 ...
 */
function newRecord(){
 	var url= "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/pss/gen3/pss_product_info/form.do";
 	var first$tree$id=document.getElementById("id_first$tree$id").value;
 	url=url+"?first$tree$id="+first$tree$id;
 	window.location.href=url;
}

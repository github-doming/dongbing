...
<form action="${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/wf/cx/wf_ins_form_info/list.do"  id="id_form_list" method="post">
...
<th
				...
				title="按[<%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . id%>]排序" style="cursor: pointer;"><%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . id%></th>
		<!-- style="display: none; cursor: pointer;" -->
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','name');"
				title="按[<%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . name%>]排序" style="cursor: pointer;"><%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . name%></th>
	<th class="class_color"><!--隐藏列事件--> <select  style="width:60px; " 
				onchange="ayTableHiddenColumn(this,'id_table',2);">
				<option value=" &nbsp;" selected="selected">&nbsp;</option>
	<option value="hit.id"><%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . id%></option>
	<option value="hit.name"><%=c.x.platform.feng.feng.wf.cx.wf_ins_form_info.alias.CxWfInsFormInfoAlias . name%></option>
			</select> <!--隐藏列事件--></th>
		</tr>
	</thead>
	<tbody>
...
function delAllRecord(id){
	var url = "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/wf/cx/wf_ins_form_info/del_all.do";
...
function delRecord(id){
	if(confirm("确定要删除吗？"))
	{
		var url = "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/wf/cx/wf_ins_form_info/del.do?id="+id;
...
function updateRecord(id){
	var url= "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/wf/cx/wf_ins_form_info/form.do?id="+id;
 	window.location.href=url;
}
/**
 * 
 ...
 */
function newRecord(){
 	var url= "${pageContext.request.contextPath}/pages/c/x/platform/admin/feng/wf/cx/wf_ins_form_info/form.do";
 	window.location.href=url;
}
	</script>
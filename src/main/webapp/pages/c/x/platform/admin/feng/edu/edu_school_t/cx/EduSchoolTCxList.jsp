...
<form action="${pageContext.request.contextPath}/c/x/platform/admin/feng/edu/edu_school_t/cx/list.do"  id="id_form_list" method="post">
...
<th
				...
				title="按[<%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . id%>]排序" style="cursor: pointer;"><%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . id%></th>
		<!-- style="display: none; cursor: pointer;" -->
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','name');"
				title="按[<%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . name%>]排序" style="cursor: pointer;"><%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . name%></th>
	<th class="class_color"><!--隐藏列事件--> <select  style="width:60px; " 
				onchange="ayTableHiddenColumn(this,'id_table',2);">
				<option value=" &nbsp;" selected="selected">&nbsp;</option>
	<option value="hit.id"><%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . id%></option>
	<option value="hit.name"><%=c.x.platform.admin.feng.edu.edu_school_t.cx.alias.EduSchoolTCxAlias . name%></option>
			</select> <!--隐藏列事件--></th>
		</tr>
	</thead>
	<tbody>
...
function delAllRecord(id){
	var url = "${pageContext.request.contextPath}/c/x/platform/admin/feng/edu/edu_school_t/cx/del_all.do";
...
function delRecord(id){
	if(confirm("确定要删除吗？"))
	{
		var url = "${pageContext.request.contextPath}/c/x/platform/admin/feng/edu/edu_school_t/cx/del.do?id="+id;
...
function updateRecord(id){
	var url= "${pageContext.request.contextPath}/c/x/platform/admin/feng/edu/edu_school_t/cx/form.do?id="+id;
 	window.location.href=url;
}
/**
 * 
 ...
 */
function newRecord(){
 	var url= "${pageContext.request.contextPath}/c/x/platform/admin/feng/edu/edu_school_t/cx/form.do";
 	window.location.href=url;
}
	</script>
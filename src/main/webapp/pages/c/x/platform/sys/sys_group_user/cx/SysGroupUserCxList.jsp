...
<form action="${pageContext.request.contextPath}/c/x/platform/sys/sys_group_user/cx/list.do"  id="id_form_list" method="post">
...
<th
				...
				title="按[<%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysGroupUserId%>]排序" style="cursor: pointer;"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias .sysGroupUserId%></th>
		<!-- style="display: none; cursor: pointer;" -->
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','SYS_USER_ID_');"
				title="按[<%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysUserId%>]排序" style="cursor: pointer;"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysUserId%></th>
		<!-- style="display: none; cursor: pointer;" -->
<th
				onclick="ayPageOrderEvent('${pageContext.request.contextPath}','SYS_GROUP_ID_');"
				title="按[<%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysGroupId%>]排序" style="cursor: pointer;"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysGroupId%></th>
	<th class="class_color"><!--隐藏列事件--> <select  style="width:60px; " 
				onchange="ayTableHiddenColumn(this,'id_table',2);">
				<option value=" &nbsp;" selected="selected">&nbsp;</option>
	<option value="hit.ID_"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias .  sysGroupUserId%></option>
	<option value="hit.SYS_USER_ID_"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysUserId%></option>
	<option value="hit.SYS_GROUP_ID_"><%=c.x.platform.sys.sys_group_user.cx.alias.SysGroupUserCxAlias . sysGroupId%></option>
			</select> <!--隐藏列事件--></th>
		</tr>
	</thead>
	<tbody>
...
function delAllRecord(id){
	var url = "${pageContext.request.contextPath}/c/x/platform/sys/sys_group_user/cx/del_all.do";
...
function delRecord(id){
	if(confirm("确定要删除吗？"))
	{
		var url = "${pageContext.request.contextPath}/c/x/platform/sys/sys_group_user/cx/del.do?id="+id;
...
function updateRecord(id){
	var url= "${pageContext.request.contextPath}/c/x/platform/sys/sys_group_user/cx/form.do?id="+id;
 	window.location.href=url;
}
/**
 * 
 ...
 */
function newRecord(){
 	var url= "${pageContext.request.contextPath}/c/x/platform/sys/sys_group_user/cx/form.do";
 	window.location.href=url;
}
	</script>
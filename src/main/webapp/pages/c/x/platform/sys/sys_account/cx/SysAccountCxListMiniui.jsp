...
html,body {
	...
	<div class="mini-fit">
		<div id="datagrid1" class="mini-datagrid"
			...
			url="${pageContext.request.contextPath}/c/x/platform/sys/sys_account/cx/listJson.do"
...
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias . sysAccountId%></div>
<div field="SYS_USER_ID_" width="100" headerAlign="center" 
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias . sysUserId%></div>
<div field="SYS_ACCOUNT_NAME_" width="100" headerAlign="center" 
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias .sysAccountName%></div>
<div field="PASSWORD_" width="100" headerAlign="center" 
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias .password%></div>
<div field="DESC_" width="100" headerAlign="center" 
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias . desc%></div>
<div field="REGISTER_TYPE_" width="100" headerAlign="center" 
dateFormat="yyyy-MM-dd" allowSort="true"><%=c.x.platform.sys.sys_account.cx.alias.SysAccountCxAlias . registerType%></div>
	...
	<script type="text/javascript">
		...
		function add() {
			var win = mini.open({
				url : "${pageContext.request.contextPath}/c/x/platform/sys/sys_account/cx/form/miniui.do",
				title : "新增员工",
				//width : 600,
				//height : 400,
		...
		function edit() {
			...
			if (row) {
				...
				//alert('id='+id);
				var win = mini.open({
					url : "${pageContext.request.contextPath}/c/x/platform/sys/sys_account/cx/form/miniui.do?id="+id,
					title : "编辑员工",
					width : 600,
					height : 400,
					allowResize : true, //允许尺寸调节
		...
		function remove() {
			...
			if (rows.length > 0) {
				if (confirm("确定删除选中记录？")) {
					...
					$.ajax({
						url : "${pageContext.request.contextPath}/c/x/platform/sys/sys_account/cx/del_all.do?id=" + id,

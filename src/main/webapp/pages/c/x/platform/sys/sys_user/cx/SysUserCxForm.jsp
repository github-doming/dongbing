...
<body>
	...
	<form id="id_form_save"
		action="${pageContext.request.contextPath}/c/x/platform/sys/sys_user/cx/save.do"
		method="post">
 ...
 %>
			...
			<tr style="display: none;">
				<td align="right"><%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias.sysUserId%></td>
				<td><input id="id_input$sys_user$SYS_USER_ID_" type="text"
					name="sys_user.sysUserId" value="${requestScope.s.sysUserId}"></td>
				<td><span id="id_span$sys_user$SYS_USER_ID_"></span></td>
			</tr>
			<tr>
				<td align="right"><%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias.sysUserName%></td>
				<td><input id="id_input$sys_user$SYS_USER_NAME_"
					placeholder="请输入<%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias . sysUserName%>"
					type="text" name="sys_user.sysUserName"
					value="${requestScope.s.sysUserName}"></td>
				<td><span id="id_span$sys_user$SYS_USER_NAME_"></span></td>
			</tr>
			<tr>
				<td align="right"><%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias.permissionGrade%></td>
				<td><input id="id_input$sys_user$PERMISSION_GRADE_"
					placeholder="请输入<%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias . permissionGrade%>"
					type="text" name="sys_user.permissionGrade"
					value="${requestScope.s.permissionGrade}"></td>
				<td><span id="id_span$sys_user$PERMISSION_GRADE_"></span></td>
			</tr>
			<tr>
				<td align="right"><%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias.desc%></td>
				<td><input id="id_input$sys_user$DESC_"
					placeholder="请输入<%=c.x.platform.sys.sys_user.cx.alias.SysUserCxAlias . desc%>"
					type="text" name="sys_user.desc" value="${requestScope.s.desc}">
				</td>
...
<script type="text/javascript">
	 ...
	 */
	function back() {
		var url = "${pageContext.request.contextPath}/c/x/platform/sys/sys_user/cx/list.do";
	...
	function check() {
		...
		//return_flag = false;
		//}
		flag = ayCheckColumn("用户名", "id_span$sys_user$SYS_USER_NAME_",
				"id_input$sys_user$SYS_USER_NAME_", "VARCHAR", "no", "256",
				"0", "0", a, true);
		if (flag) {
		} else {
			return_flag = false;
		...
		//return_flag = false;
		//}
		flag = ayCheckColumn("数据权限等级", "id_span$sys_user$PERMISSION_GRADE_",
				"id_input$sys_user$PERMISSION_GRADE_", "INT", "yes", "10", "0",
				"0", a, true);
		if (flag) {
		} else {
			return_flag = false;
		}
		flag = ayCheckColumn("描述", "id_span$sys_user$DESC_",
				"id_input$sys_user$DESC_", "VARCHAR", "yes", "1024", "0", "0",
				a, true);
		if (flag) {
		} else {
			return_flag = false;
